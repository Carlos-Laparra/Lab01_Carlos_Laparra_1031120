...
namespace ArbolB
{
    public class ArbolB<T>
    {
        ...
        private void find2(T dato, Node<T> Actual)
        {
            ValorRepetido = false;
            bool ciclo = false;
            if (Actual.tienehijos())
            {
                ...
                while (Actual.nodosInternos[i] != null && ciclo != true)
                {
                    ...
                    {
                        ValorRepetido = true;
                        Actual.nodosInternos[i].Value = dato;
                        ciclo = true;
                    }
                    else if (comparador.Invoke(dato, Actual.nodosInternos[i].Value) == -1)
                    {
                        find2(dato, Actual.nodosInternos[i].Left);
                        ciclo = true;
                    }
                    else if (Actual.nodosInternos[i + 1] == null)
                    {
                        find2(dato, Actual.nodosInternos[i].Right);
                        ciclo = true;
                    }
                    i++;
                }
            ...
            else
            {
                for (int x = 0; x < n - 1; x++)
                {
                    if (Actual.nodosInternos[x] != null)
                    {
                        ...
                        {
                            ValorRepetido = true;
                            Actual.nodosInternos[x].Value = dato;
                        }
                    }
        ...
        public void patch(T dato)
        {
            find2(dato,root);
        }
        internal void recorridohoja(T dato, Node<T> Actual)
        {
        ...
        internal void inorder(T dato, Node<T> Actual)
        {
            for (int i = 0; i < Actual.valueslength(); i++)
            {
                if (Actual.tienehijos())
                {
                    if (i == 0)
                    {
                        inorder(dato, Actual.nodosInternos[i].Left);
                        if (comparador.Invoke(dato, Actual.nodosInternos[i].Value) == 0)
                        {
                            ValorRepetido = true;
                            pre.Add(Actual.nodosInternos[i].Value);
                    ...
                    inorder(dato, Actual.nodosInternos[i].Right);

                }
                else
                {
                    recorridohoja(dato, Actual);
